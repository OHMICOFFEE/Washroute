...
import { WASH_PACKAGE_LABELS, WASH_PRICES, EXTRA_PRICES, EXTRA_LABELS, EXPRESS_FEE, STANDARD_FEE, CONCIERGE_FEE } from '@/lib/utils/pricing'
import { Pencil, Check, X } from 'lucide-react'
import toast from 'react-hot-toast'

interface PriceRow { code: string; name: string; price: number; category: string }
...
export default function AdminServicesPage() {
  ...
  function PriceCell({ code, base }: { code: string; base: number }) {
    ...
    return editing === code ? (
      ...
      <div className="flex items-center gap-2">
        <span className="font-bold text-sm" style={{ color: changed ? '#ff9500' : 'var(--brand-primary)' }}>
          R{current}
        </span>
  ...
  return (
    <div className="space-y-6 anim-fadeup stagger">
      <div className="pt-2">
        ...
        <p className="caption mt-1">Tap the ✏️ icon to edit any price. Changes apply immediately.</p>
      </div>
      ...
      {/* Fees */}
      <div>
        <h2 className="heading mb-3">Collection Fees</h2>
        <div className="list-group">
          <div className="list-item">
            <span className="text-sm flex-1" style={{ color: 'var(--text-primary)' }}>Standard Pickup (1hr lead time)</span>
            <PriceCell code="fee_standard" base={STANDARD_FEE} />
          </div>
          <div className="list-item">
            <span className="text-sm flex-1" style={{ color: 'var(--text-primary)' }}>Express Pickup (30min earlier)</span>
            <PriceCell code="fee_express" base={EXPRESS_FEE} />
          </div>
      ...
      {Object.keys(store.customPrices).length > 0 && (
        <button
          ...
          onClick={() => { Object.keys(store.customPrices).forEach(k => store.updatePricing(k, -1)); toast.success('Prices reset to defaults') }}>
