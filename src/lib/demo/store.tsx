...
export interface DemoBooking {
  ...
  // Concierge actuals (driver fills in)
...
export function DemoStoreProvider({ children }: { children: ReactNode }) {
  const [data, setData] = useState<{
    ...
    customerProfile: CustomerProfile | null
    invoices:     Invoice[]
  }>({ bookings: [], drivers: [], messages: [], customPrices: {}, credits: [], staff: [], timeEntries: [], customerProfile: null, invoices: [] })

  useEffect(() => {
    const loaded = load()
    if (loaded) {
      setData({
        credits: [], staff: [], timeEntries: [], customerProfile: null, invoices: [],
        customPrices: {}, messages: [], bookings: [], drivers: [],
        ...loaded,
      })
    } else {
      setData({
  ...
  const updatePricing = useCallback((code: string, price: number) => {
    update(d => {
      if (price < 0) {
        // price < 0 means remove override (reset to default)
  ...
  const useCredit = useCallback((id: string) => {
    update(d => ({
      ...d,
      credits: d.credits.map(c => c.id === id ? { ...c, used: true, used_at: now() } : c),
    }))
  }, [])
  ...
  const getActiveCredits = useCallback((customerName?: string) => {
    ...
    return data.credits.filter(c =>
      !c.used &&
      new Date(c.expires_at) > now2 &&
      (!customerName || c.customer_name === customerName)
    )
  }, [data.credits])
  ...
  const clockOut = useCallback((entryId: string, breakMins = 0) => {
    update(d => ({
      ...
      timeEntries: d.timeEntries.map(e =>
        e.id === entryId ? { ...e, clock_out: now(), break_mins: breakMins } : e
      ),
    }))
  }, [])
  ...
  const approveTimeEntry = useCallback((id: string) => {
    update(d => ({
      ...d,
      timeEntries: d.timeEntries.map(e => e.id === id ? { ...e, approved: true, approved_by: 'Admin' } : e),
    }))
  }, [])

  const getStaffEntries = useCallback((staffId: string, month?: string) => {
    return data.timeEntries.filter(e =>
      e.staff_id === staffId && (!month || e.date.startsWith(month))
    )
  }, [data.timeEntries])
  ...
  const createInvoice = useCallback((bookingId: string): string => {
    ...
    // Separate VAT-able and zero-rated (fuel) amounts
    const fuelAmount = booking.fuel_refill && booking.fuel_amount
      ? parseFloat(booking.fuel_amount) || 0 : 0
    ...
    // Wash package (VAT inclusive)
    const washLabel = booking.wash_package.replace(/_/g, ' ').replace(/(?:^|\s)\S/g, l => l.toUpperCase())
    items.push({ description: washLabel, amount: booking.total * 0.6 })

    // Extras
    booking.extras.forEach(e => items.push({ description: e, amount: 0 }))

    // Pickup fee
    if (booking.pickup_type === 'express') {
      items.push({ description: 'Express Pickup Fee', amount: 150 })
    } else {
      items.push({ description: 'Standard Pickup Fee', amount: 100 })
    }

    // Concierge (VAT inclusive)
    ...
    // Fuel (ZERO RATED - no VAT on petrol in SA)
    if (fuelAmount > 0) {
      items.push({
        description: `Fuel Refill — ${booking.fuel_type} @ ${booking.fuel_station} (Zero-rated)`,
        amount: fuelAmount,
        vat_note: 'zero_rated',
      })
      ...
      if (booking.fuel_credit && booking.fuel_credit > 0) {
        items.push({
          description: `Fuel Credit (overpay refund to next wash)`,
          amount: -booking.fuel_credit,
          vat_note: 'zero_rated',
        })
      }
    }
